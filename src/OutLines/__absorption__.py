...
# absorption
def absorp(w,u,beta,vini,geo,cone,par,VF,DP):
    if geo[1] == 1. :
...
def phi_int(beta,incl,tO,tC,vdisk,vini,par,VF,DP,u,umin,umax,cone):
    ...
    if umin >= umax:
        ...
        # return the integral
        return fixed_quad(absorp,umin,umax,args=(u,beta,vini,geo,cone,par,VF,DP),n=256)[0]
# calculate unnormalized profile for a sphere or bicone
def calc_phi(w,w0,vinf,beta,incl,tO,tC,xdisk,vini,vapr,*par,VF='BetaCAK',DP='PowerLaw'):
