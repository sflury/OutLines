...
def calc_limits(w,w0,vinf,vapr,beta,VelocityField,Inflow=False):
    ...
    if Inflow :
        umin[w[ind]<w0] = array(list(map(partial(solve_u,beta,VelocityField),y[ind][w[ind]<w0])))
        cone[w[ind]<w0] = 'post'
    else:
        umin[w[ind]>w0] = array(list(map(partial(solve_u,beta,VelocityField),y[ind][w[ind]>w0])))
        cone[w[ind]>w0] = 'post'
...
# emission per unit velocity
# cos t = v_obs / v_inf = u / v_inf = u if u in v_inf units
# u = cos t
# sin t = sqrt(1-cos^2 t) = sqrt(1-u^2)
def emiss(w,u,beta,geo,cone,par,VelocityField,DensityProfile):
    if geo[1] == 1.:
        bl = 1.
    else:
    ...
    return bl*n[DensityProfile](w,beta,VelocityField,*par)**2
...
def phi_int(beta,incl,tO,tC,vdisk,par,VelocityField,DensityProfile,u,umin,umax,cone):
    ...
    if umin >= umax:
        ...
        # return the integral
        return fixed_quad(emiss,umin,umax,args=(u,beta,geo,cone,par,VelocityField,DensityProfile),n=256)[0]
